...
public class CreateFormsJdbc{
	...
	private void createProjectMajor(Applier applier){
		String sql="insert into project_major (applierUid, yearCreated, refereeString,secretLevel) values(?,?,?,'∑«√‹')";
		int year = Calendar.getInstance().get(Calendar.YEAR);
		jdbcTemplateObject.update(sql,applier.getUid(),year,applier.getOwner());
		logger.info(sql);
	}
